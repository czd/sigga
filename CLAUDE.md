...
- **Convex Auth** (`@convex-dev/auth`) with Google OAuth and family codes (`ConvexCredentials`). Google enforces the `ALLOWED_EMAILS` whitelist via the `createOrUpdateUser` callback; family codes are admin-issued per-person codes — issuing a code IS the authorization. No invite system. **Note for future credentials providers (SMS OTP, etc.): `ConvexCredentials.authorize` is the *only* gate — `createOrUpdateUser` never fires for it, so any whitelist/authorization check must live inside `authorize`.**
...
- **All `Intl.DateTimeFormat` calls in `src/` must pass `timeZone: APP_TIME_ZONE`** (exported from `src/lib/formatDate.ts` as `"Atlantic/Reykjavik"`). Date-input parsing (e.g., `toDatetimeLocal`/`fromDatetimeLocal`) must use `Date.UTC` / `getUTC*` accessors so the wall-clock string is treated as Reykjavík time. A future component that omits `timeZone` falls back to the *environment's* zone — the server's during SSR, the device's after hydration — which can produce a wrong initial render and a hydration mismatch rather than reliable Reykjavík time. There is no lint rule for this, so it must be caught in QA.
- **Convex test files must use the `*.test.ts` naming convention** — the Convex bundler skips any file whose basename contains more than one dot (e.g. `foo.test.ts`). Single-dot names like `fooTest.ts` are picked up as entrypoints, fail to bundle (they import `vitest` / `convex-test`, which are devDependencies not available in the Convex runtime), and break `convex deploy`. This failure only manifests in Vercel CI, not in local `bun run test`.
