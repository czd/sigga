...
- **All `Intl.DateTimeFormat` calls in `src/` must pass `timeZone: APP_TIME_ZONE`** (exported from `src/lib/formatDate.ts` as `"Atlantic/Reykjavik"`). Date-input parsing (e.g., `toDatetimeLocal`/`fromDatetimeLocal`) must use `Date.UTC` / `getUTC*` accessors so the wall-clock string is treated as Reykjavík time. A future component that omits `timeZone` will silently display the server's local time, not the user's — there is no lint rule for this, so it must be caught in QA.
