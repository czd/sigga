...
**Intentional cursor divergence:** `sinceLastVisit` takes a client-side `cursorMs` sourced from localStorage (per-device, per-user key in `SinceLastVisit.tsx`), while `unreadCount` reads the server-side `journalReads.lastSeenTime` high-water mark. These intentionally serve different purposes — the dashboard feed shows "what you haven't seen on this device" while the nav badge shows "what hasn't been seen across any device". Do not "fix" this divergence; it is confirmed design (Nic, 2026-06-02). The mismatch (badge clears when Dagbók is opened on any device; feed resets only when this device visits the dashboard) is an acceptable trade-off.
