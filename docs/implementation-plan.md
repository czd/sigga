...
- File must be `proxy.ts` at project root
- Export must be named `proxy`, not `middleware`
- Use `convexAuthNextjsMiddleware` but adapt the export name

### Files to create/modify

- `convex/auth.ts` — Convex Auth config with Google OAuth provider
...
- `proxy.ts` — Auth redirect logic
- `.env.local` — `CONVEX_DEPLOYMENT`, `NEXT_PUBLIC_CONVEX_URL`

### Exit criteria

- Can sign in with a whitelisted Google account → lands on dashboard
- Non-whitelisted account → sees Icelandic rejection message
